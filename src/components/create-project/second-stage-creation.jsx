...
function SecondStageCreation() {
  ...
  useEffect(() => {
    const fetchResources = async () => {
      try {
        ...
        const response = await Api.getProducts(token, { dtoType: "ProductDto", page_size: 200 });
        ...
        setResourceQuantities(projectResources ? projectResources : initialQuantities);
  ...
  const handleQuantityChange = (resourceId, resourceN, value) => {
    ...
    setResourceQuantities((prevQuantities) => {
      ...
      const index = prevQuantities.findIndex((resource) => resource.id === resourceId);
            ...
            <Col md={6} className="offset-md-3">
              ...
              <div style={{ maxHeight: "240px", overflowY: "auto" }}>
                {filteredResources.map((resource) => {
                  ...
                  const quantityObj = resourceQuantities.find((q) => q.id === resource.id) || { quantidade: 0 };
                  return (
                    <div key={resource.id} style={{ display: "flex", justifyContent: "space-between", marginBottom: "10px" }}>
                      <span>{resource.name}</span>
                      <input
                        type="number"
                        min="0"
                        value={quantityObj.quantity}
                        onChange={(e) => handleQuantityChange(resource.id, resource.name, e.target.value)}
                        style={{ width: "60px" }}
                      />
                    </div>
                  );
